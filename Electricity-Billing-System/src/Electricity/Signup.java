    ...
    public void actionPerformed(ActionEvent ae){
        if(ae.getSource() == b1){
            ...
            try{
                ...
                boolean flag=false;
                while(rs.next()){
                    if(username.equals(rs.getString("username"))) { flag=true; break;}
                    if(meter.equals(rs.getString("meter_no")) && "".equals(rs.getString("username"))  )cnt++;
                    System.out.println("Cnt :"+cnt);
                }
                
                if(flag){ JOptionPane.showMessageDialog(null, "UserName is already present, Please select another!!!"); }
                else if(cnt==0){ JOptionPane.showMessageDialog(null, "OOPS!!!  Meter number is not available");}
                else{
