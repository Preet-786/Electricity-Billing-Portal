...
public class NewCustomer extends JFrame implements ActionListener{
    ...
    public void actionPerformed(ActionEvent ae){
        if(ae.getSource() == b1){
            ...
            try{
                boolean flag = false;
                if( (email.length()<11) || !("@gmail.com".equals(email.substring(email.length()-10)))){
                    flag = true;
                    msg+="Email is incorrect\n";
                }
                if(phone.length()!=10 ) { flag =true; msg+="Phone No. should be 10 digit\n";}
                else{
                    for(int i=0;i<phone.length();i++){
                        if(phone.charAt(i)<'0' || phone.charAt(i)>'9' ) { 
                            flag = true; msg+="Phone No. should be numeric\n"; break;
                        }
                    }
                }
                if(flag){JOptionPane.showMessageDialog(null,msg);}
                else{
                    Conn c1 = new Conn();
